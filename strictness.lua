...
local yanji = fk.CreateTriggerSkill{
  ...
  can_trigger = function(self, event, target, player, data)
    if target == player then
      if event == fk.EventPhaseStart then
        ...
        U.checkZhengsu(player, target, self.name)
      end
    end
  end,
  on_cost = function(self, event, target, player, data)
  ...
  on_use = function(self, event, target, player, data)
    ...
    if event == fk.EventPhaseStart then
      U.startZhengsu(player, player, self.name, "#yanji-choice")
    else
      local choices = {"draw2"}
...
local zhengjun = fk.CreateTriggerSkill{
  ...
  on_use = function(self, event, target, player, data)
    ...
    if event == fk.EventPhaseStart then
      U.startZhengsu(player, player, self.name, "#zhengjun-choice")
    else
      local choices = {"draw2"}
      if player:isWounded() then
        table.insert(choices, 1, "recover")
      ...
      if #to > 0 then
        ...
        U.rewardZhengsu(player, to, reward, self.name)
      end
    end
  end,
}
...
Fk:loadTranslationTable{
  ...
  ["zhengjun"] = "整军",
  [":zhengjun"] = "出牌阶段开始时，你可以进行“<a href='zhengsu_desc'>整肃</a>”，弃牌阶段结束后，若“<a href='zhengsu_desc'>整肃</a>”成功，你获得“<a href='zhengsu_desc'>整肃</a>”奖励，然后你可以令一名其他角色也获得“<a href='zhengsu_desc'>整肃</a>”奖励。",
...
local houfeng = fk.CreateTriggerSkill{
  ...
  on_use = function(self, event, target, player, data)
    ...
    U.startZhengsu(player, target, self.name, "#houfeng-choice::"..target.id)
...
local houfeng_delay = fk.CreateTriggerSkill{
  ...
  can_trigger = function(self, event, target, player, data)
    ...
    U.checkZhengsu(player, target, "houfeng")
  end,
  on_cost = Util.TrueFunc,
  on_use = function(self, event, target, player, data)
    local room = player.room
    ...
    U.rewardZhengsu(player, target, reward, "houfeng")
    if not player.dead then
      U.rewardZhengsu(player, player, reward, "houfeng")
    end
  end,
}
...
Fk:loadTranslationTable{
  ...
  ["houfeng"] = "厚俸",
  [":houfeng"] = "每轮限一次，你攻击范围内一名角色出牌阶段开始时，你可以令其“<a href='zhengsu_desc'>整肃</a>”；你与其共同获得“<a href='zhengsu_desc'>整肃</a>”奖励。",
  ["zj__juxiang"] = "拒降",
